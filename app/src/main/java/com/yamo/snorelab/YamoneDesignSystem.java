...
public final class YamoneDesignSystem {
    ...
    public static LinearLayout card(Context context, Palette palette) {
        return card(context, palette, palette.surface, palette.border);
    }

    public static LinearLayout card(Context context, Palette palette, int fill, int border) {
        LinearLayout card = new LinearLayout(context);
        card.setOrientation(LinearLayout.VERTICAL);
        // Renewal 1: visible 2dp outline so cards do not look flat or unfinished.
        card.setBackground(roundedBorder(fill, border, 18, 2, context));
    ...
    public static TextView badge(Context context, String label, Palette palette) {
        TextView badge = text(context, label, 10, palette.primaryStrong, true);
        badge.setGravity(Gravity.CENTER);
        badge.setPadding(dp(context, 9), dp(context, 5), dp(context, 9), dp(context, 5));
        badge.setBackground(roundedBorder(palette.surfaceSoft, palette.border, 999, 1, context));
        return badge;
    }
    ...
    public static TextView button(Context context, String label, boolean primary, Palette palette) {
        ...
        button.setBackground(roundedBorder(fill, primary ? palette.primaryStrong : palette.border, 16, 1, context));
