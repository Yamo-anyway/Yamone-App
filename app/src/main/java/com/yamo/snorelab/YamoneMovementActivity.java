...
public class YamoneMovementActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= 33) unregisterSystemBack = Api33.register(this, this::dispatchSystemBack);
        if (!mockupAssetsInstalled()) { showMissingAssetNotice(); return; }
        ...
        safeRoot.setOnApplyWindowInsetsListener((v, insets) -> {
            ...
            if (Build.VERSION.SDK_INT >= 30) {
                android.graphics.Insets topInsets = insets.getInsets(WindowInsets.Type.statusBars() | WindowInsets.Type.displayCutout());
                android.graphics.Insets bottomInsets = insets.getInsets(WindowInsets.Type.navigationBars() | WindowInsets.Type.mandatorySystemGestures());
                android.graphics.Insets horizontalInsets = insets.getInsets(WindowInsets.Type.systemBars() | WindowInsets.Type.displayCutout());
                top = topInsets.top; bottom = bottomInsets.bottom; left = horizontalInsets.left; right = horizontalInsets.right;
            } else {
            ...
            lp.leftMargin = left; lp.topMargin = top; lp.rightMargin = right; lp.bottomMargin = bottom + dp(BOTTOM_TOUCH_SAFETY_DP);
    ...
    private final class NativeBridge {
        @android.webkit.JavascriptInterface public void setTheme(String theme) { runOnUiThread(() -> applySystemBars("pink".equals(theme))); }
        @android.webkit.JavascriptInterface public void finishApp() { runOnUiThread(YamoneMovementActivity.this::exitConfirmed); }
    }

    @SuppressWarnings("deprecation")
    @Override public void onBackPressed() { dispatchSystemBack(); }
    ...
    private void dispatchSystemBack() {
        ...
        backPending = true;
        webView.evaluateJavascript("Boolean(window.yamoneAndroidBack && window.yamoneAndroidBack())", value -> {
            backPending = false;
